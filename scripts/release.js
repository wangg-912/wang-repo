...
const runIfNotDry = isDryRun ? dryRun : run
  ...
  ; (async function main() {
    ...
    step('\nRunning wang4 tests...')
    await run('yarn', ['workspace', 'wang4', 'test'])

    step('\nBuilding wang4...')
    await run('yarn', ['workspace', 'wang4', 'build'])

    step('\nUpdate wang4 version...')
    await run('yarn', [
      'workspace',
      'wang4',
      'version',
    ...
    step('\nUpdating wang4 cross dependencies...')
    ...
    await runIfNotDry(
      ...
      {
        cwd: path.resolve(__dirname, '../packages/wang4'),
        stdio: 'pipe'
      }
    )
